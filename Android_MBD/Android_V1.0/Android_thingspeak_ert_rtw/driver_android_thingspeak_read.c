...
void initThingSpeakread(const int channelID, const char* readAPIKey,int field){
    
    JNIEnv *pEnv;
    (*cachedJvm)->AttachCurrentThread(cachedJvm, &pEnv, NULL);
    
    methodId = (*pEnv)->GetMethodID(pEnv, cachedMainActivityCls, "initThingSpeakReadConnection","(ILjava/lang/String;I)V");
    if (methodId == NULL)
        return; /* initThingSpeakConnection method not found */
    
    jstring jstrKey = (*pEnv)->NewStringUTF(pEnv, readAPIKey);
    (*pEnv)->CallVoidMethod(pEnv, cachedActivityObj, methodId, channelID, jstrKey, field);
